...
public class TaskServiceImpl implements TaskService {
    ...
    public UniversalResponseBody<Task> taskCheckIn(Double checkLongitude, Double checkLatitude, Integer taskId, Integer userId) {
        ...
        if (status.equals(UserTaskStatusEnum.USER_CHECKED.getMsg())) {
        ...
        // 更新打卡地点、打卡状态和获得的鸽子蛋数量
        TaskMember taskMember = new TaskMember();
        taskMember.setTaskId(taskId);
        taskMember.setUserId(userId);
        taskMember.setUseCheckLongitude(checkLongitude);
        taskMember.setUserCheckLatitude(checkLatitude);
        taskMember.setUserTaskStatus(UserTaskStatusEnum.USER_CHECKED.getMsg());
